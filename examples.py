...
def grad_echo(
    ...
    slice_duration = (
        rf_tend - rf_tstart
    ) + 2 * trap_ramp_duration  # includes rise, plateau and fall
    ...
    tr_total_time = (
        readout_tstart + readout_duration + tr_wait + 7000
    )  # start-finish TR time

    def grad_echo_tr(tstart, pamp):
        gvxt, gvxa = trapezoid(
            slice_amp, slice_duration, trap_ramp_duration, trap_ramp_pts
        )
        gvyt, gvya = trapezoid(pamp, phase_duration, trap_ramp_duration, trap_ramp_pts)

        gvzt1 = trapezoid(
            readout_amp, readout_duration / 2, trap_ramp_duration, trap_ramp_pts
        )
        gvzt2 = trapezoid(
            -readout_amp, readout_duration / 2, trap_ramp_duration, trap_ramp_pts
        )
        ...
        value_dict = {
            ...
            "tx0": (
                np.array([rf_tstart, rf_tend, rx_tcentre - 10, rx_tcentre + 10])
                + tstart,
                np.array([rf_amp, 0, dbg_sc * (1 + 0.5j), 0]),
            ),
            "tx1": (
                np.array([rx_tstart + 15, rx_tend - 15]) + tstart,
                np.array([dbg_sc * pamp * (1 + 0.5j), 0]),
            ),
            ...
            "rx1_en": (
                np.array([rx_tstart, rx_tend]) + tstart,
                np.array([1, 0]),
            ),  # acquire on RX1 for example too
            "rx_gate": (np.array([rx_tstart, rx_tend]) + tstart, np.array([1, 0])),
            "tx_gate": (
                np.array([rf_tstart - tx_gate_pre, rf_tend + tx_gate_post]) + tstart,
                np.array([1, 0]),
            ),
        }
...
def turbo_spin_echo(
    ...
    def rf_wf(tstart, echo_idx):
        ...
        if echo_idx == 0:
            ...
            return (
                ...
                np.array([pi2_phase, 0, pi_phase]) * rf_amp,
            )
    ...
    def phase_grad_wf(tstart, echo_idx):
        t1, a1 = trap_cent(
            tstart + (echo_duration - phase_grad_interval) / 2,
            phase_amps[echo_idx - 1],
            phase_grad_duration,
            trap_ramp_duration,
            trap_ramp_pts,
        )
    ...
    echo_duration * (echos_per_tr + 1) + tr_pause_duration
...
def radial(trs=36, plot_rx=False, init_gpa=False, plot_sequence=False):
    ...
    def radial_tr(tstart, th):
        ...
        value_dict = {
            ...
            "tx0": (
                np.array([rf_tstart, rf_tend, rx_tstart + 15, rx_tend - 15]),
                np.array([rf_amp, 0, dbg_sc * (gx + gy * 1j), 0]),
            ),
            "tx1": (
                np.array([rx_tstart + 15, rx_tend - 15]),
                np.array([dbg_sc * (gx + gy * 1j), 0]),
            ),
            "grad_vz": (np.array([gradz_tstart]), np.array([gx])),
            "grad_vy": (np.array([grady_tstart]), np.array([gy])),
            "rx0_en": (np.array([rx_tstart, rx_tend]), np.array([1, 0])),
            "rx_gate": (np.array([rx_tstart, rx_tend]), np.array([1, 0])),
            "tx_gate": (
                np.array([rf_tstart - tx_gate_pre, rf_tend + tx_gate_post]),
                np.array([1, 0]),
            ),
        }
