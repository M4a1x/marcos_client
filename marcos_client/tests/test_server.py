...
class ServerTest(unittest.TestCase):
    ...
    def test_version(self):
        full_test = (
            True  # test a range of different versions; otherwise just the current one
        )
        debug_replies = False
        ...
        def diff_info(client_ver):
            return {
                "infos": [
                    "Client version {:d}.{:d}.{:d}".format(*client_ver)
    ...
    def test_idle(self):
        ...
        real = send_packet(
            construct_packet({"are_you_real": 0}, self.packet_idx), self.s
        )[4]["are_you_real"]
        ...
        def check_status():
            ...
            if (
                (status_latch_reg & fhdo_err)
                or (status_latch_reg & ocra1_err)
                or (status_latch_reg & ocra1_data_lost)
            ):
                warnings.warn(
        ...
        self.assertEqual(
            ...
            [
                ...
                0,
                version_full,
                {"regstatus": [0, adc_value, 0, 0, 0, buf_empties, 0]},
                {},
            ],
        )
    ...
    def test_bus(self):
        ...
        if real == "hardware":
            ...
            times = (
                ...
                131.0,
                158.5,
            )  # numerical operation, bus read, bus write on hardware
            loops = 1000000
        ...
        if real == "hardware":
            self.assertAlmostEqual(
                null_t / 1e3, times[0] * loops_norm, delta=deltas[0] * loops_norm
            )  # 1 flop takes ~1.5 ns on average
            self.assertAlmostEqual(
                read_t / 1e3, times[1] * loops_norm, delta=deltas[1] * loops_norm
            )  # 1 read takes ~141.9 ns on average
            self.assertAlmostEqual(
                write_t / 1e3, times[2] * loops_norm, delta=deltas[2] * loops_norm
            )  # 1 write takes ~157.9 ns on average
        elif real == "simulation":
    ...
    def test_net(self):
        ...
        if real == "hardware":
            pass  # upper-bound times for network transfers
        elif real == "simulation":
            pass  # upper-bound times for network transfers
        elif real == "software":
            pass  # upper-bound times for network transfers
        construct_packet({"test_net": 10}, self.packet_idx)
        # VN: continue here
    ...
    def test_fpga_clk_partial(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                {
                    "errors": [
                        "you only provided some FPGA clock control words; check you're providing all 3"
                    ]
                },
            ],
        )
    ...
    def test_several_okay(self):
        packet = construct_packet(
            {
                "lo_freq": 0x7000000,  # floats instead of uints
                "tx_div": 10,
                "rx_div": 250,
                "tx_size": 32767,
    ...
    def test_several_some_bad(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                {
                    "errors": [
                        "TX divider outside the range [1, 10000]; check your settings",
    ...
    def test_grad_adc(self):
        ...
        for iw in init_words:
            ...
            send_packet(
                construct_packet({"direct": 0x01000000 | (iw & 0xFFFF)}), self.s
            )
        ...
        self.assertEqual(
            expected, readback[1:]
        )  # ignore 1st word, since it depends on the history of ADC transfers
    ...
    def test_leds(self):
        ...
        for k in range(256):
            ...
            reply = send_packet(packet, self.s)
            self.assertEqual(reply, [reply_pkt, 1, 0, version_full, {"direct": 0}, {}])
    ...
    def test_mar_mem(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                version_full,
                {"mar_mem": 0},
                {"infos": [f"mar mem data bytes copied: {mar_mem_bytes:d}"]},
            ],
        )
