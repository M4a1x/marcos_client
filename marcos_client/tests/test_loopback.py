...
if __name__ == "__main__":
    ...
    if (
        False
    ):  # VN: this stuff belongs in a more sophisticated test, not test_loopback. TODO: refactor/remove
        gamma = 42570000  # Hz/T

        # value for tabletopMRI  gradient coil
        B_per_m_per_current = (
            0.02  # T/m/A, approximate value for tabletop gradient coil
        )

        # values for gpa fhdo
        gpa_current_per_volt = (
            3.75  # A/V, theoretical value for gpa fhdo without 2.5 V offset!
        )
        max_dac_voltage = 2.5
        ...
        max_Hz_per_m = (
            max_dac_voltage * gpa_current_per_volt * B_per_m_per_current * gamma
        )
    ...
    tx_arr, grad_arr, cb, params = ps.assemble(
        "../ocra-pulseq/test_files/test_loopback.seq", byte_format=False
    )

    exp = ex.Experiment(
        samples=params["readout_number"],  # TODO: get information from PSAssembler
        lo_freq=lo_freq,
        tx_t=tx_t,
        rx_t=params["rx_t"],
    ...
    if do_jitter_test:
        ...
        plt.title(
            "passing loopback data ({:d}/{:d}, {:.2f}%)".format(
                lgd, lgd + lbd, 100 * lgd / (lgd + lbd)
            )
        )
        plt.grid(True)
        ...
        plt.ylabel("loopback rx amplitude")
        plt.title(
            "failing loopback data ({:d}/{:d}, {:.2f}%)".format(
                lbd, lgd + lbd, 100 * lbd / (lgd + lbd)
            )
        )
        plt.grid(True)
