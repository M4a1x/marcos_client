...
class OCRA1:
    def __init__(self, server_command_f, max_update_rate=0.1):
        ...
        self.spi_div = (
            int(np.floor(1 / (spi_cycles_per_tx * max_update_rate * grad_clk_t))) - 1
        )
        if self.spi_div > 63:
    ...
    def init_hw(self):
        init_words = [
            ...
            0x02400004,
            0x04400004,
            0x07400004,  # reset DACs to power-on values
            0x00200002,
            0x02200002,
            0x04200002,
            0x07200002,  # set internal amplifier
            0x00100000,
            0x02100000,
            0x04100000,
            0x07100000,  # set outputs to 0
        ]
    ...
    def float2bin(self, grad_data, channel=0):
        ...
        return (
            np.round(131071.49 * gd_cal).astype(np.uint32) & 0x3FFFF
        )  # 2's complement

    def bin2float(self, grad_bin):
        return (((grad_bin & 0x3FFFF).astype(np.int32) ^ (1 << 17)) - (1 << 17)).astype(
            np.int32
        ) / 131072
...
class GPAFHDO:
    def __init__(self, server_command_f, max_update_rate=0.1):
        """max_update_rate is in MSPS for updates on a single channel; used to choose the SPI clock divider"""
        fhdo_max_update_rate = (
            max_update_rate * 4
        )  # single-channel serial, so needs to be faster

        spi_cycles_per_tx = 30  # actually 24, but including some overhead
        self.spi_div = (
            int(np.floor(1 / (spi_cycles_per_tx * fhdo_max_update_rate * grad_clk_t)))
            - 1
        )
        if self.spi_div > 63:
            self.spi_div = 63  # max value, < 100 ksps
    ...
    def init_hw(self):
        ...
        init_words = [
            ...
            0x400D0600,
            0x400F0600,
            0x40110600,  # input ranges for each ADC channel
            0x00088000,
            0x00098000,
            0x000A8000,
            0x000B8000,  # set each DAC channel to output 0
        ]
    ...
    def update_on_msb_writes(self, upd, spi_div=None):
        ...
        self.server_command(
            {"direct": 0x00000000 | (2 << 0) | (spi_div << 2) | (0 << 8) | (upd << 9)}
        )
    ...
    def write_dac(self, channel, value, gated_writes=True):
        ...
        msbs = (
            0x0008 | channel | (channel << 9)
        )  # extra channel word for gpa_fhdo_iface, not sure if it's currently used
        value_msbs = value >> 16
        if (value_msbs != 0) and (value_msbs != msbs):
            warnings.warn(
                "You requested channel {:d}, which does not match that of the binary word provided (0x{:0x}). Are you using channels consistently?".format(
                    channel, value
                )
            )
    ...
    def calculate_corrected_dac_code_old(self, channel, dac_code):
        ...
        return np.round(
            np.interp(
                self.expected_adc_code_from_dac_code(dac_code),
                self.gpaCalValues[channel],
                self.dac_values,
            )
        ).astype(np.uint32)
    ...
    def amp2grad(self, ampere):
        ...
        # return dac_code
        return ampere / (
            self.gpa_current_per_volt * (v_max - v_ref)
        )  # full-scale grad val is +/- 1
    ...
    def calibrate(
        ...
        for chan in channels:
            ...
            if test_cal:
                ...
                plt.ylabel(
                    "Residuals (observed - expected grad vals) (normalised, [-1, 1])"
                )
                plt.show()
            else:
                ...
                try:
                    ...
                    warnings.warn(
                        "Poly fit failed due to numerical problems -- perhaps no current could be output."
                    )
                ...
                if coeff > 1.05 or coeff < 0.95:
                    warnings.warn(
                        "Poly slope coefficient {:f} for chan {:d} is outside [0.95, 1.05]; will not be used. Make sure the coils are connected to the GPA-FHDO and the system is correctly powered.".format(
                            coeff, chan
                        )
                    )
                else:
            ...
            self.write_dac(
                chan, self.float2bin(0, chan, cal=True)
            )  # restore to precise midpoint
    ...
    def calibrate_old(
        ...
        self.dac_values = np.round(
            np.linspace(
                self.ampere_to_dac_code(-max_current),
                self.ampere_to_dac_code(max_current),
                num_calibration_points,
            )
        )
        ...
        for channel in range(self.grad_channels):
            ...
            for k, dv in enumerate(self.dac_values):
                ...
                gpaCalRatios[k] = self.gpaCalValues[channel][
                    k
                ] / self.expected_adc_code_from_dac_code(dv)
            ...
            if np.amax(gpaCalRatios) > 1.01 or np.amin(gpaCalRatios) < 0.99:
                print(
                    "Calibration for channel {:d} seems to be incorrect. Calibration factor is {:f}. Make sure a gradient coil is connected and gpa_current_per_volt value is correct.".format(
                        channel, np.amax(gpaCalRatios)
                    )
                )
            if plot:
