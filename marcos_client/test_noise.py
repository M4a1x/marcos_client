...
def test_jitter(
    ...
    rf_data = (
        rf_pulse_start_time
        + np.array([0, rf_pulse_length, rf_interval, rf_pulse_length + rf_interval]),
    ...
    grad_data = (
        ...
        + np.array(
            [0, grad_pulse_length, grad_interval, grad_pulse_length + grad_interval]
        ),
    ...
    event_dict = {
        ...
        "grad_vz2": grad_data,
        "tx_gate": (
            tx_gate_start_time + np.array([0, tx_gate_length]),
            np.array([1, 0]),
        ),
    }

    exp = ex.Experiment(
        lo_freq=rf_pulse_freq,
        gpa_fhdo_offset_time=gpa_fhdo_offset_time,
        **experiment_kwargs,
    )
...
if __name__ == "__main__":
    ...
    if True:
        ...
        test_jitter(
            loops=loops,
            rf_pulse_start_time=10.5,
            rf_interval=interval,
            rf_amp=amp,
            grad_interval=interval,
            grad_amp=amp,
