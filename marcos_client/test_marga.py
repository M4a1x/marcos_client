...
def run_manual_test(data, interval=0.001, timeout=20):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        for _k in range(int(timeout / interval)):
            time.sleep(interval)
...
def leds():
    ...
    addr = 0
    for k in range(256):
        raw_data[addr] = insta(IWAIT, delay - 1)
        addr += 1  # -1 because the next instruction takes 1 cycle
        # raw_data[addr] = insta(IWAIT, 0x300000); addr += 1 # -1 because the next instruction takes 1 cycle
        raw_data[addr] = instb(GATES_LEDS, 0, (k & 0xFF) << 8)
        addr += 1
    # go idle
    raw_data[addr] = insta(IFINISH, 0)
    addr += 1
...
def example_tr_loop():
    ...
    dds0_phase_step = np.round(2**31 / fpga_clk_freq_MHz * lo_freq0).astype(
        np.uint32
    )  # 31b phase accumulator in marga
    dds1_phase_step = np.round(2**31 / fpga_clk_freq_MHz * lo_freq1).astype(
        np.uint32
    )  # 31b phase accumulator in marga
    ...
    raw_data[addr] = instb(DDS0_PHASE_LSB, 3, 0)
    addr += 1
    raw_data[addr] = instb(DDS0_PHASE_MSB, 2, 0x8000)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_LSB, 1, 0)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_MSB, 0, 0x8000)
    addr += 1

    # set the phase increment, start phase going
    raw_data[addr] = instb(DDS0_PHASE_LSB, 3, dds0_phase_step & 0xFFFF)
    addr += 1
    raw_data[addr] = instb(DDS0_PHASE_MSB, 2, dds0_phase_step >> 16)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_LSB, 1, dds1_phase_step & 0xFFFF)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_MSB, 0, dds1_phase_step >> 16)
    addr += 1

    # allow the new phase to propagate through the chain before setting nonzero I/Q values
    raw_data[addr] = insta(IWAIT, 100)
    addr += 1  # max I value

    # configure RX settings: both channels to use DDS source 0
    # reset CICs
    raw_data[addr] = rx_ctrl(0, 0, 0, 0, 0, 0, 0, 0, 0)
    addr += 1
    # take them out of reset
    raw_data[addr] = rx_ctrl(40, 0, 0, 0, 0, dds_demod_ch, dds_demod_ch, 1, 1)
    addr += 1
    # set control buses to new rates (immediately for now)
    raw_data[addr] = instb(RX0_RATE, 0, cic0_decimation)
    addr += 1
    raw_data[addr] = instb(RX1_RATE, 0, cic1_decimation)
    addr += 1

    # briefly signal that there's a new rate
    raw_data[addr] = rx_ctrl(40, 0, 0, 1, 1, dds_demod_ch, dds_demod_ch)
    addr += 1
    raw_data[addr] = rx_ctrl(0, 0, 0, 0, 0, dds_demod_ch, dds_demod_ch)
    addr += 1

    for _k in range(tr_loops):
        ...
        raw_data[addr] = insta(IWAIT, 100 - initial_block_delay - 5)
        addr += 1

        addr_tx_start = addr
        raw_data[addr] = instb(GATES_LEDS, initial_block_delay + 1, 0x1)
        addr += 1  # turn on TX gate
        raw_data[addr] = instb(TX0_I, initial_block_delay - 1, 0xC000)
        addr += 1
        raw_data[addr] = instb(TX0_Q, initial_block_delay - 2, 0x3FFF)
        addr += 1
        raw_data[addr] = instb(TX1_I, initial_block_delay - 3, 0xC000)
        addr += 1
        raw_data[addr] = instb(TX1_Q, initial_block_delay - 4, 0x3FFF)
        addr += 1
        ...
        raw_data[addr] = insta(
            IWAIT,
            int(np.round(wait * fpga_clk_freq_MHz))
            + 1
            - initial_block_delay
            - addr
            + addr_tx_start,
        ...
        addr += 1
        raw_data[addr] = instb(GATES_LEDS, initial_block_delay + 1, 0x0)
        addr += 1
        # turn off TX gate
        raw_data[addr] = instb(TX0_I, initial_block_delay - 1, 0x0)
        addr += 1
        raw_data[addr] = instb(TX0_Q, initial_block_delay - 2, 0x0)
        addr += 1
        raw_data[addr] = instb(TX1_I, initial_block_delay - 3, 0x0)
        addr += 1
        raw_data[addr] = instb(TX1_Q, initial_block_delay - 4, 0x0)
        addr += 1
        addr_tx_end = addr

        # Wait then acquire some data, wait some more, stop acquisition
        raw_data[addr] = insta(IWAIT, 100 - 4 - addr + addr_tx_end)
        addr += 1  # wait 100 cycles
        raw_data[addr] = rx_ctrl(40, 1, 1, 0, 0, dds_demod_ch, dds_demod_ch)
        addr += 1
        raw_data[addr] = insta(IWAIT, 500 - 5)
        addr += 1  # wait 500 cycles
        raw_data[addr] = rx_ctrl(40, 0, 0, 0, 0, dds_demod_ch, dds_demod_ch)
        addr += 1
...
def tx_short():
    ...
    # allow the new phase to propagate through the chain before setting nonzero I/Q values
    raw_data[addr] = insta(IWAIT, 20)
    addr += 1  # max I value

    steps = 300
    pattern0 = np.hstack(
        [
            np.linspace(1, 0, steps // 3),
            np.linspace(1, -1, steps // 3),
            np.linspace(-1, 1, steps // 3),
        ]
    )
    pattern1 = np.hstack(
        [
            np.linspace(0, 1, steps // 3),
            np.linspace(1, 0, steps // 3),
            np.linspace(0, -1, steps // 3),
        ]
    )
    ...
    for k in range(steps):
        hv0 = tdata0[k] & 0xFFFF
        hv1 = tdata1[k] & 0xFFFF
        raw_data[addr] = instb(TX0_I, 3, hv0)
        addr += 1
        raw_data[addr] = instb(TX0_Q, 2, hv0)
        addr += 1
        raw_data[addr] = instb(TX1_I, 1, hv1)
        addr += 1
        raw_data[addr] = instb(TX1_Q, 0, hv1)
        addr += 1

    # do mid-scale output, and change frequency
    raw_data[addr] = instb(TX0_I, 3, 0x4000)
    addr += 1
    raw_data[addr] = instb(TX0_Q, 2, 0x4000)
    addr += 1
    raw_data[addr] = instb(TX1_I, 1, 0x4000)
    addr += 1
    raw_data[addr] = instb(TX1_Q, 0, 0x4000)
    addr += 1

    # wait for 3us; shortened delay by 4 cycles for the next instructions to be right on time
    raw_data[addr] = insta(IWAIT, int(3 * fpga_clk_freq_MHz) - 4)
    addr += 1

    dds2_phase_step = np.round(2**31 / fpga_clk_freq_MHz * lo_freq2).astype(
        np.uint32
    )  # 31b phase accumulator in marga
    dds3_phase_step = np.round(2**31 / fpga_clk_freq_MHz * lo_freq3).astype(
        np.uint32
    )  # 31b phase accumulator in marga

    # switch frequency on both channels simultaneously, no reset
    raw_data[addr] = instb(DDS0_PHASE_LSB, 3, dds2_phase_step & 0xFFFF)
    addr += 1
    raw_data[addr] = instb(DDS0_PHASE_MSB, 2, dds2_phase_step >> 16)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_LSB, 1, dds3_phase_step & 0xFFFF)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_MSB, 0, dds3_phase_step >> 16)
    addr += 1

    raw_data[addr] = insta(IWAIT, int(4 * fpga_clk_freq_MHz))
    addr += 1

    # go idle
    raw_data[addr] = instb(TX0_I, 3, 0)
    addr += 1
    raw_data[addr] = instb(TX0_Q, 2, 0x8000)
    addr += 1
    raw_data[addr] = instb(TX1_I, 1, 0)
    addr += 1
    raw_data[addr] = instb(TX1_Q, 0, 0x8000)
    addr += 1
    raw_data[addr] = insta(IFINISH, 0)
    addr += 1
    raw_data = raw_data[:addr]  # truncate
...
def rx_short():
    ...
    assert dds0_phase_step < 2**31, "DDS frequency outside of valid range"
    raw_data[addr] = instb(DDS0_PHASE_LSB, 1, dds0_phase_step & 0xFFFF)
    addr += 1
    raw_data[addr] = instb(DDS0_PHASE_MSB, 0, dds0_phase_step >> 16)
    addr += 1

    # configure RX settings: both channels to use DDS source 3 (DC), decimation of 10
    # reset CICs
    ## raw_data[addr] = instb(RX0_CTRL, 0, 0x0000); addr += 1
    raw_data[addr] = rx_ctrl(0, 0, 0, 0, 0, 0, 0)
    addr += 1
    ...
    raw_data[addr] = rx_ctrl(50, 1, 0, 0, 0, dds_demod_ch, 0)
    addr += 1
    raw_data[addr] = instb(RX0_RATE, 0, cic_decimation)
    addr += 1
    # briefly signal that there's a new rate
    ## raw_data[addr] = instb(RX0_CTRL, 50, 0xc000 | cic_decimation | (dds_demod_ch << 12) ); addr += 1
    raw_data[addr] = rx_ctrl(50, 1, 0, 1, 0, dds_demod_ch, 0)
    addr += 1
    ...
    raw_data[addr] = rx_ctrl(0, 1, 0, 0, 0, dds_demod_ch, 0)
    addr += 1
    # raw_data[addr] = instb(RX1_CTRL, 0, 0xb000 | cic_decimation); addr += 1

    # just acquire data for a while
    raw_data[addr] = insta(IWAIT, acquisition_ticks - 1)
    addr += 1

    # reset RX again
    ## raw_data[addr] = instb(RX0_CTRL, 1, 0x3000); addr += 1
    raw_data[addr] = rx_ctrl(1, 0, 0, 0, 0, dds_demod_ch, 0)
    addr += 1
    # raw_data[addr] = instb(RX1_CTRL, 0, 0x3000); addr += 1

    # turn off DDS0
    raw_data[addr] = instb(DDS0_PHASE_LSB, 1, 0)
    addr += 1
    raw_data[addr] = instb(DDS0_PHASE_MSB, 0, 0)
    addr += 1
...
def loopback(cic0_decimation=7, cic1_decimation=10):
    ...
    # zero the phase increment initially and reset the phase
    raw_data[addr] = instb(DDS0_PHASE_LSB, 7, 0)
    addr += 1
    raw_data[addr] = instb(DDS0_PHASE_MSB, 6, 0x8000)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_LSB, 5, 0)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_MSB, 4, 0x8000)
    addr += 1

    # set the phase increment, start phase going
    raw_data[addr] = instb(DDS0_PHASE_LSB, 0, dds0_phase_step & 0xFFFF)
    addr += 1
    raw_data[addr] = instb(DDS0_PHASE_MSB, 0, dds0_phase_step >> 16)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_LSB, 0, dds1_phase_step & 0xFFFF)
    addr += 1
    raw_data[addr] = instb(DDS1_PHASE_MSB, 0, dds1_phase_step >> 16)
    addr += 1
    ...
    # configure RX settings: both channels to use DDS source 3 (DC), decimation of 10
    # reset CICs
    raw_data[addr] = rx_ctrl(0, 0, 0, 0, 0, dds_demod_ch, dds_demod_ch, 0, 0)
    addr += 1
    # raw_data[addr] = instb(RX1_CTRL, 0, 0x0000); addr += 1
    # take them out of reset later
    raw_data[addr] = rx_ctrl(40, 0, 0, 0, 0, dds_demod_ch, dds_demod_ch)
    ...
    raw_data[addr] = rx_ctrl(0, 1, 1, 0, 0, dds_demod_ch, dds_demod_ch)
    addr += 1
    ...
    if False:
        ...
        raw_data[addr] = rx_ctrl(0, 0, 0, 1, 0, dds_demod_ch, dds_demod_ch)
        addr += 1
        raw_data[addr] = rx_ctrl(0, 0, 0, 0, 0, dds_demod_ch, dds_demod_ch)
        addr += 1
        # same for channel 1:
        raw_data[addr] = instb(RX1_RATE, 0, cic1_decimation)
        addr += 1
        raw_data[addr] = rx_ctrl(0, 0, 0, 0, 1, dds_demod_ch, dds_demod_ch)
        addr += 1
        raw_data[addr] = rx_ctrl(0, 0, 0, 0, 0, dds_demod_ch, dds_demod_ch)
        addr += 1
        # wait 100 cycles and reset both CICs, then wait 100 more and take them out of reset and start acquisition
        raw_data[addr] = rx_ctrl(100, 0, 0, 0, 0, dds_demod_ch, dds_demod_ch, 0, 0)
        addr += 1
        raw_data[addr] = rx_ctrl(100, 1, 1, 0, 0, dds_demod_ch, dds_demod_ch)
        addr += 1
    ...
    raw_data[addr] = insta(IWAIT, int(extra_time * fpga_clk_freq_MHz))
    addr += 1
    ...
    raw_data[addr] = instb(TX0_I, 3, 0)
    addr += 1
    raw_data[addr] = instb(TX0_Q, 2, 0x0000)
    addr += 1
    raw_data[addr] = instb(TX1_I, 1, 0)
    addr += 1
    raw_data[addr] = instb(TX1_Q, 0, 0x0000)
    addr += 1
    raw_data[addr] = insta(IFINISH, 0)
    addr += 1
    # raw_data = raw_data[:addr] # truncate
...
def long_loopback():
    ...
    addr += 1
    # briefly signal that there's a new rate
    raw_data[addr] = rx_ctrl(40, 1, 1, 1, 1, dds_demod_ch, dds_demod_ch)
    addr += 1
    # raw_data[addr] = instb(RX1_CTRL, 49, 0xf000 | cic_decimation); addr += 1
    # end the new rate flag (the buffers are not empty so no offset time is needed)
    raw_data[addr] = rx_ctrl(0, 1, 1, 0, 0, dds_demod_ch, dds_demod_ch)
    ...
    for k in range(phase_points):
        raw_data[addr] = instb(TX0_I, 0, cb[k])
        addr += 1
        raw_data[addr] = instb(TX0_Q, 0, sb[k])
        addr += 1
        raw_data[addr] = instb(TX1_I, 0, cb[k])
        addr += 1
        raw_data[addr] = instb(TX1_Q, 0, sb[k])
        addr += 1
        raw_data[addr] = insta(IWAIT, sine_ts)
        addr += 1
    ...
    # go idle
    raw_data[addr] = instb(TX0_I, 0, 0)
    addr += 1
    raw_data[addr] = instb(TX0_Q, 0, 0x8000)
    addr += 1
    raw_data[addr] = instb(TX1_I, 0, 0)
    addr += 1
    raw_data[addr] = instb(TX1_Q, 0, 0x8000)
    addr += 1
    raw_data[addr] = insta(IFINISH, 0)
...
if __name__ == "__main__":
    ...
    if False:  # multiple trials, verify mean against saved
        ...
        rx0im, rx0qm, rx1im, rx1qm = (
            rx0_i.mean(0),
            rx0_q.mean(0),
            rx1_i.mean(0),
            rx1_q.mean(0),
        )
        rx0rms, rx1rms = np.sqrt(rx0im**2 + rx0qm**2), np.sqrt(
            rx1im**2 + rx1qm**2
        )
    ...
    if False:
        ...
        plt.xlabel("sample")
        plt.title(f"loopback, {trials:d} trials")
        plt.show()
