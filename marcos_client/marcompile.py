...
def col2buf(col_idx, value):
    ...
    if col_idx in (1, 2, 3, 4):  # TX
        ...
        if grad_board == "gpa-fhdo":
            ...
            grad_chan = col_idx - 9
            val_full = (
                value << 2 | 0x00100000 | (grad_chan << 25) | 0x01000000
        ...
        buf_idx = (
            lo_lsb_buf,
            lo_lsb_buf + 1,
        )  # DDS[0,1,2]_PHASE_LSB, DDS[0,1,2]_PHASE_MSB
...
def dict2bin(
    ...
    for k, vals in sd.items():  # iterate over dictionary keys
        ...
        buf_idces, values, masks = col2buf(
            col_idx, vals[1]
        )  # single element or array of values
...
def cl2bin(
    changelist, changelist_grad, initial_bufs=np.zeros(MARGA_BUFS, dtype=np.uint16)
):
            ...
            if grad_board == "ocra1":  # simultaneous with another grad update
                if msb:
                    if num_chgs[
                        ...
                        grad_vals[
                            :
                        ] = grad_vals_old  # revert the last known buffer values
                # else:
    ...
    if len(removed_instruction_warnings) > max_removed_instructions:
        ...
        warnings.warn(
            "NOTE: Fewer than {:d} removed-instruction warnings will not be printed -- keep this in mind when searching for the root cause.".format(
                max_removed_instructions
            )
        )
    ...
    for event in changes:
        ...
        while excess_dtime_tmp > 2:  # delay of 3 or more cycles needed
            wait_time = min(
                excess_dtime_tmp, COUNTER_MAX + 3
