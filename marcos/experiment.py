...
class Experiment:
    ...
    def set_lo_freq(self, lo_freq):
        ...
        self._dds_phase_steps = np.round(
            2**31 / fpga_clk_freq_MHz * np.array(lo_freq)
        ).astype(np.uint32)
        self._lo_freqs = (
            self._dds_phase_steps * fpga_clk_freq_MHz / (2**31)
        )  # real LO freqs -- TODO: print for debugging
    ...
    def flo2int(self, seq_dict):
        ...
        def times_us(farr):
            ...
            return np.round(fpga_clk_freq_MHz * farr).astype(
                np.int64
    ...
    def add_flodict(self, flodict, append=True):
        ...
        assert (
            self._csv is None
        ), "Cannot replace the dictionary for an Experiment class created from a CSV"
    ...
    def compile(self):
        ...
        rx0_words, self._rx0_cic_factor = fc.cic_words(
            self._rx_divs[0], self._set_cic_shift
        )
        rx1_words, self._rx1_cic_factor = fc.cic_words(
...
def test_rx_scaling(
    ...
    expt = Experiment(
        lo_freq=lo_freq,
        rx_t=rx_periods[0] / fpga_clk_freq_MHz,
        fix_cic_scale=False,
        set_cic_shift=False,
